...
class DescargaTembloresUSGS extends Proceso {
    ...
    async descargaIncremental() {
        ...
        // USGS puede incorporar o revisar eventos después de su tiempo de ocurrencia.
        // El upsert por id hace seguro releer una ventana reciente.
    ...
    async iniciaProcesamientoTemblores(resultado) {
        const descargados = Number(resultado?.descargados || 0);
        if (!descargados) {
            await this.logInfo("No se inicia procesamiento de temblores descargados: no hubo nuevos registros");
            return;
        }

        const options = {
            origen: this.codigo,
            descargaEjecucionId: this.ejecucionId || null,
            descargados,
            bloques: Number(resultado?.bloques || 0),
            totalBaseDatos: Number(resultado?.totalBaseDatos || 0)
        };

        try {
            const ack = await procesaTembloresDescargados.ejecutaAsync(options);
            resultado.procesamientoTemblores = ack;
            await this.logInfo("Procesamiento de temblores descargados iniciado", ack);
        } catch (error) {
            const mensaje = error?.message || error?.toString() || "ERR_INICIA_PROCESAMIENTO_TEMBLORES";
            resultado.procesamientoTemblores = { estado: "error", mensaje };
            await this.logWarning("No se pudo iniciar procesamiento de temblores descargados", mensaje);
        }
    }
