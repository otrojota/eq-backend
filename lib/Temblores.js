...
class Temblores extends ZModule {
    ...
    init() {
        this.oasDeclareSchema("Temblor", {
            ...
            properties: {
                _id: { type: "string", description: "Identificador estable compuesto por fuente e id de origen" },
                idOrigen: { type: "string", description: "Identificador original del evento en la fuente" },
                ...
                ubicacionFuente: { type: "string", description: "Descripcion de ubicacion entregada por la fuente" },
                urlFuente: { type: "string", description: "URL del registro original o del catalogo consultado" },
        ...
        this.registerEndPoint("GET", "temblores/puntos-normalizados", this.getPuntosTembloresNormalizados, {
            ...
            parameters: [
                ...
                { name: "desde", in: "query", required: false, description: "Fecha UTC o timestamp minimo", schema: { type: "string" } },
                { name: "hasta", in: "query", required: false, description: "Fecha UTC o timestamp maximo", schema: { type: "string" } },
                { name: "codigoPlaca", in: "query", required: false, description: "Placa asociada al borde tectonico", schema: { type: "string" } },
                { name: "limit", in: "query", required: false, description: "Cantidad maxima de puntos", schema: { type: "integer" } }
            ],
            responses: {
                "200": {
        ...
        this.registerEndPoint("GET", "temblores/:id", this.getTemblor, {
            ...
            parameters: [
                { name: "id", in: "path", required: true, description: "Identificador compuesto del evento", schema: { type: "string" } }
            ],
            responses: {
                "200": {
    ...
    normalizaTemblor(temblor) {
        ...
        if (!temblor.fuente) throw "ERR_TEMBLOR_SIN_FUENTE";
    ...
    async getPuntosTembloresNormalizados(decada, desde, hasta, codigoPlaca, limit) {
        const col = await mongo.collection("temblores");
        const query = this.creaQueryPuntosTemblores(decada, desde, hasta, {
            valido: true,
            tiempo: { $ne: null },
        ...
        if (codigoPlaca) query.coordenadasPlacas = { $elemMatch: { codigoPlaca } };
        const nLimit = this.normalizaLimitPuntos(limit);
