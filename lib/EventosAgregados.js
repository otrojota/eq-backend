...
class EventosAgregados extends ZModule {
    ...
    init() {
        this.oasDeclareSchema("EventoAgregado", {
            type: "object",
            required: ["_id", "placasAsociadas", "tiempoInicio", "tiempoFin", "punto", "energiaAcumulada"],
            properties: {
                _id: { type: "string" },
                codigoPlaca: { type: "string" },
                placasAsociadas: { type: "array", items: { type: "string" } },
    ...
    parametrosConsulta(incluyeDecada = false) {
        ...
        parametros.push(
            ...
            { name: "normalizado", in: "query", required: false, schema: { type: "boolean" } },
    ...
    creaQuery(decada, desde, hasta, codigoPlaca) {
        ...
        if (decada !== undefined && decada !== null && decada !== "") {
            ...
            inicioRango = Date.UTC(nDecada, 0, 1);
            finRango = Date.UTC(nDecada + 10, 0, 1) - 1;
        }
        const desdeTiempo = this.parseTiempo(desde);
        const hastaTiempo = this.parseTiempo(hasta);
        if (desdeTiempo !== null) inicioRango = inicioRango === null ? desdeTiempo : Math.max(inicioRango, desdeTiempo);
        if (hastaTiempo !== null) finRango = finRango === null ? hastaTiempo : Math.min(finRango, hastaTiempo);
        if (inicioRango !== null) query.tiempoFin = { $gte: inicioRango };
        if (finRango !== null) query.tiempoInicio = { $lte: finRango };
        if (codigoPlaca) query.placasAsociadas = codigoPlaca;
        return query;
    }
    ...
    async getPuntosEventosAgregados(decada, desde, hasta, codigoPlaca, normalizado, limit) {
        const col = await mongo.collection("eventos_agregados");
        const query = this.creaQuery(decada, desde, hasta, codigoPlaca);
        const campoPunto = normalizado === true || normalizado === "true" ? "$puntoNormalizado" : "$punto";
        query[campoPunto.slice(1) + ".latitud"] = { $type: "number" };
        query[campoPunto.slice(1) + ".longitud"] = { $type: "number" };
        return await col.aggregate([
            { $match: query },
            { $sort: { tiempoInicio: 1 } },
            { $limit: this.normalizaLimit(limit, 500000) },
            { $project: { _id: 1, latitud: `${campoPunto}.latitud`, longitud: `${campoPunto}.longitud`, normalizado: { $literal: campoPunto === "$puntoNormalizado" }, tiempo: "$tiempoInicio", tiempoFin: 1, energia: "$energiaAcumulada", magnitud: "$magnitudPrincipal", radioKm: { $ifNull: ["$estadisticas.radioDistribucionKm", "$parametrosCluster.radioKm"] }, radioAsociacionKm: "$parametrosCluster.radioKm", codigoPlaca: 1, placasAsociadas: 1, cantidadTemblores: 1 } }
        ]).toArray();
    }
    ...
    validaEvento(evento) {
        if (!evento || !evento._id || !(evento.placasAsociadas || []).length || !evento.punto) throw "ERR_EVENTO_AGREGADO_INVALIDO";
        return evento;
    }
