...
class EventosAgregadosPronosticados extends ZModule {
    ...
    async getPuntos(codigoPlaca, desde, hasta, vigente, historico) {
        ...
        if (!esHistorico) {
            const ayer = new Date(); ayer.setUTCHours(0, 0, 0, 0); ayer.setUTCDate(ayer.getUTCDate() - 1);
            q.vigente = true;
            q.tiempoMedio = { $gte: ayer.getTime() };
        } else {
            const tiempo = {};
        ...
        return await (await mongo.collection("eventos_agregados_pronosticados")).aggregate([{ $match: q }, { $sort: { tiempoMedio: 1, codigoPlaca: 1, orden: 1 } }, { $project: { _id: 1, pronosticoId: 1, codigoPlaca: 1, orden: 1, vigente: 1, tiempo: "$tiempoMedio", latitud: "$ubicacion.punto.latitud", longitud: "$ubicacion.punto.longitud", energia: "$energiaAcumulada.p50", radioKm: "$radioDistribucionKm.p50", probabilidadBorde: "$ubicacion.probabilidadBorde" } }]).toArray();
    }
